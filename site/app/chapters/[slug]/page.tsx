    ...
    <main className="reader-shell">
      <aside className="chapter-sidebar">
        ...
        <nav aria-label="Chapter table of contents">
          ...
          <ol>{chapter.headings.map(heading => {
      ...
      <article className="chapter-reader">
        ...
        <nav className="chapter-pagination" aria-label="Chapter navigation">
          ...
          {next ? <Link href={`/chapters/${next.slug}`}><span>Next</span><strong>{next.title} →</strong></Link> : <Link href="/#library"><span>Next</span><strong>Explore the roadmap →</strong></Link>}
        </nav>
      </article>
    </main>
  );
